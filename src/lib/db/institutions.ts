...
// Lista apenas instituições públicas (aparecem na lista numerada do WhatsApp)
...
// Busca por nome/slug em TODAS as instituições ativas (públicas e ocultas)
...
export async function getCurrentInstitutionForUser(userId: string): Promise<Institution | null> {
  ...
  const { data: inst } = await getSupabaseAdmin()
    .from('institutions')
    .select('*')
    .eq('id', data.current_institution_id)
    .maybeSingle();
...
export async function setCurrentInstitutionForUser(userId: string, institutionId: string): Promise<void> {
  ...
  const { error } = await getSupabaseAdmin()
    .from('user_institution_preferences')
    .upsert(
      { user_id: userId, current_institution_id: institutionId, origin_institution_id: originId, updated_at: new Date().toISOString() },
      { onConflict: 'user_id' }
    );
